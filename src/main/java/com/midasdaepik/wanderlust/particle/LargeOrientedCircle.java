...
public class LargeOrientedCircle extends TextureSheetParticle {
    ...
    public void render(VertexConsumer pBuffer, Camera pRenderInfo, float pPartialTicks) {
        this.alpha = (float) Math.cos((double) this.age / this.lifetime / 2 * Math.PI);
    ...
    public void tick() {
        if (this.age++ >= this.lifetime) {
            ...
            this.quadSize = (float) Math.sin((double) this.age / this.lifetime / 2 * Math.PI) * (this.end_scale - this.start_scale) + this.start_scale;
        }
    }
